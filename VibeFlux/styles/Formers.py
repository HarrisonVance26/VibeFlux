...
from ..base.Sets import loadSettings
...
def applyBackground(widget, background_path):
    ...
    if object_name:
        begin_marker = f"/* VibeFlux managed background begin: {object_name} */"
        end_marker = f"/* VibeFlux managed background end: {object_name} */"
        ...
        background_qss = f"#{object_name} {{ {background_qss} }}"
        background_qss = f"{begin_marker}\n{background_qss}\n{end_marker}"
    widget.setStyleSheet((current_qss.rstrip() + "\n" if current_qss else "") + background_qss)
