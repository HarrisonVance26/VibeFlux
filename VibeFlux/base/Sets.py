...
def _append_widget_qss(widget, widget_name, qss_body):
    """
    Append object-scoped QSS without discarding styles already applied to the widget.
    """
    begin_marker = f"/* VibeFlux managed background begin: {widget_name} */"
    end_marker = f"/* VibeFlux managed background end: {widget_name} */"
    block = f"{begin_marker}\n#{widget_name} {{ {qss_body} }}\n{end_marker}"
    current_qss = widget.styleSheet() if hasattr(widget, "styleSheet") else ""
    while begin_marker in current_qss and end_marker in current_qss:
        before, rest = current_qss.split(begin_marker, 1)
        _, after = rest.split(end_marker, 1)
        current_qss = before.rstrip() + "\n" + after.lstrip()
    if current_qss:
        widget.setStyleSheet(current_qss.rstrip() + "\n" + block)
    else:
        widget.setStyleSheet(block)
        ...
        if 'background' in settings:
            ...
            if pathExists(background_path):
                ...
                _append_widget_qss(
                    widget,
                    widget_name,
                    f"border-image: url({background_path}) 0 0 0 0 stretch stretch; "
